###############################################################################
#  Raman analysis of gypsum vs. marble
#  PCA + PLS-DA + VIP + loadings, every band mapped back to a chemical bond
#
#  Data: data/rock_raman_2class_raw.csv
#        5 spectra of gypsum (CaSO4.2H2O) + 5 of marble (calcite, CaCO3)
#
#  This script does exactly the same thing as python/rock_raman_tutorial.ipynb
#  and as the orange-spectra PLS-DA widget in Orange. The three agree to
#  better than 1e-6.
#
#  One add-on package required:  install.packages("pls")
#  Matrix ships with R, so nothing else is needed.
...
## ===========================================================================
## 1. Read the data
##    One row per spectrum, one column per wavenumber. From the third column
##    onwards the column name IS the wavenumber.
##    check.names = FALSE is essential: without it R rewrites "150.004" as
##    "X150.004", as.numeric() then returns NA, and nothing raises an error.
## ===========================================================================
...
cat("spectra =", nrow(X), " wavenumber points =", ncol(X), "\n")
cat("range =", round(range(wn), 1), "cm-1\n")
cat("classes:", paste(lev, table(grp), sep = " x ", collapse = ", "), "\n")
# 664 variables for 10 samples -- variables outnumber samples 66 to one.
# That is the p >> n regime; section 7 deals with what it implies.


## ===========================================================================
## 2. The three preprocessing tools
## ===========================================================================

## 2-1 ALS baseline: find a curve that is as smooth as possible while
##     hugging the data from below.
als <- function(y, lambda = 1e4, p = 0.01, niter = 15) {
  m  <- length(y)
  D  <- diff(Diagonal(m), differences = 2)     # 2nd-difference matrix = curvature
  DD <- lambda * crossprod(D)
  w  <- rep(1, m)
  for (i in 1:niter) {
    W <- Diagonal(x = w)
    z <- as.numeric(solve(W + DD, w * y))
    w <- p * (y > z) + (1 - p) * (y <= z)      # down-weight points above (peaks)
  }
  z
}

## 2-2 Savitzky-Golay: fit a polynomial in a sliding window.
##     deriv = 1 gives the first derivative.
...
## 2-3 SNV: centre and scale each spectrum by its own mean and SD,
##     removing overall brightness differences.
snv <- function(y) (y - mean(y)) / sd(y)

BASE <- t(apply(X, 1, als))
Xbl  <- t(apply(X - BASE, 1, sg))              # <- the main analysis uses this
...
## ---------------------------------------------------------------------------
##  gypsum  CaSO4.2H2O : 413/493 (SO4 v2), 620/670 (v4), 1008 (v1, strongest),
##                       1135 (v3)
##  calcite CaCO3      : 156/282 (lattice), 712 (CO3 v4), 1086 (v1, strongest),
##                       1435 (v3)
## ===========================================================================
BANDS <- data.frame(
...
# Thresholds: S/N >= 3 counts as detected, >= 10 as reliable. Both diagnostic
# bands are far above threshold here.
# But look at the h419 column: 419 appears in BOTH groups, and calcite has no
# vibrational mode there. That is an instrument artefact.


## ===========================================================================
## 4. PCA (unsupervised)
##    center = TRUE is mandatory. scale. = FALSE -- do NOT standardise each
##    wavenumber column for spectra, or pure-noise wavenumbers get promoted to
##    the same importance as the 1086 band.
## ===========================================================================
...
for (nm in names(SETS)) {
  ...
  for (g in lev) { k <- grp == g; h <- chull(s[k, 1:2])   # convex hull; with n=5 a
    polygon(s[k, 1][c(h, h[1])], s[k, 2][c(h, h[1])],     # confidence ellipse would
            border = COL[g], col = adjustcolor(COL[g], .10), lty = 2) }  # be dishonest
  if (nm == "Raw") legend("topright", lev, col = COL, pch = PCH, bty = "n", cex = .85)
}
dev.off()

## Loadings: the score plot says who resembles whom, the loading plot says
## which peak is responsible.
...
## ===========================================================================
## 5. PLS-DA (supervised)
##    Encode the classes as a 0/1 dummy matrix, regress it on X with PLS2,
##    and predict by taking the largest output column.
##    method = "oscorespls" is NIPALS PLS2 -- the same algorithm as
##    orange-spectra's plsda_fit(). "simpls" does not return loading.weights,
##    so VIP cannot be computed from it.
## ===========================================================================
...
## ===========================================================================
## 6. VIP: which wavenumbers is the model using?
##    VIP_j = sqrt( p * sum_a( ssy_a * (w_ja/||w_a||)^2 ) / sum_a ssy_a )
##    VIP > 1 is the conventional threshold, because the mean of the squared
##    VIP scores is exactly 1. This is the same formula orange-spectra uses.
## ===========================================================================
vip <- function(m, nc) {
  W  <- m$loading.weights[, 1:nc, drop = FALSE]
  T  <- m$scores[,          1:nc, drop = FALSE]
  Q  <- m$Yloadings[,       1:nc, drop = FALSE]
  SS <- colSums(Q^2) * colSums(T^2)              # Y variance explained per component
  Wn <- sweep(W, 2, sqrt(colSums(W^2)), "/")
  sqrt(nrow(W) * rowSums(sweep(Wn^2, 2, SS, "*")) / sum(SS))
}
NC <- 2
V  <- vip(mod, NC)
cat("\nWavenumbers with VIP > 1:", sum(V > 1), "of", length(V), "\n")
cat("Top 15 by VIP:\n")
print(head(data.frame(wn = round(wn, 1), VIP = round(V, 3))[order(-V), ], 15), row.names = FALSE)
# Ranks 2, 5 and 7 are 419 / 422 / 416 -- the instrument artefact. A high VIP
# only means the model used that variable, not that it means anything.
...
## The two PLS loadings, for comparison with Orange's Loadings output
cat("\nSix largest |loading| per component:\n")
for (a in 1:2) {
  L <- mod$loadings[, a]; o <- order(-abs(L))[1:6]
  cat(sprintf("  p%d: %s\n", a, paste(sprintf("%.1f(%+.3f)", wn[o], L[o]), collapse = "  ")))
}
# p2's four largest entries all sit in the 419 artefact band: the second latent
# variable is essentially an "instrument artefact component". It carries only
# 3.98% of the X variance and is not needed for classification at all.
...
##     The model is rebuilt on every fold -- feeding the held-out sample to an
##     already-fitted model would prove nothing.
...
cat("\nLeave-one-out confusion matrix (2 LV):\n"); print(cm)
obs <- mean(pred == as.character(grp)); cat("accuracy =", obs, "\n")

## 7-2 Exact permutation test.
##     Five versus five allows only C(10,5) = 252 label assignments, so we
##     enumerate all of them and obtain an exact p-value rather than an estimate.
cmb  <- combn(10, 5)
...
cat(sprintf("\nExact permutation test: observed = %.3f, null mean = %.3f, p = %d/%d = %.4f\n",
            obs, mean(accs), sum(accs >= obs), length(accs), pval))
# Only two assignments reach 100%: the true labelling and its complete swap
# (the model simply learns the reversed mapping). So p = 2/252 = 0.0079 is the
# floor for a five-versus-five design -- it cannot go lower, however good the
# signal. Worth knowing while designing the experiment, not after analysing it.
...
## ===========================================================================
## 8. Export, for the Python and Orange comparisons
##    These are the same files the Chinese-commented script writes; the code is
##    identical, so the outputs are byte-for-byte the same.
## ===========================================================================
...
## Orange native .tab: line 1 names, line 2 types, line 3 flags
...
cat("\nDone. Figures in ../fig/, tables in ../data/.\n")
###############################################################################
